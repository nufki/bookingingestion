...
1. **Kafka Listener**
   ...
   fun consumeBooks(records: List<ConsumerRecord<String, BooksOuterClass.Books?>>) = runBlocking {
       val messages = records.mapNotNull { it.value() }
       bookingIngestionService.ingestBatch(messages)
   }
   ```

   - Consumes protobuf `Books` messages from the configured topic.
   - Delegates to `BookingIngestionService` to persist a batch of messages.

2. **Persistence**

   - `BookingIngestionService` maps incoming protobuf messages to DB entities / jOOQ records and writes them to MySQL.
   - Flyway manages the schema (e.g. `BOOKS`, `EVT_PKT` tables).
   - jOOQ is generated from DDL scripts via the `jooq-codegen-maven` plugin.
...
3. **Query API**
   ...
   - Returns a list of `BookingDto` objects for a given position ID.
   - Uses reactive jOOQ access under the hood.

---

## Configuration

The project uses **profiles** with `application.yaml` and `application-dev.yaml`.
...
innuvation:
  bookingingestion-service:
    ...
    books:
      input-topic-name: bookingingestion-books-input
```
...
3. **Produce test `Books` messages**
   ...
   There is a small Kotlin producer that:
   ...
   ```kotlin
   val producer = getKafkaProducer<Books>(
